...
bounding_box = {'top': 0, 'left': 0, 'width': 960, 'height': 1280}
sct = mss()

frame0 = sct.grab(bounding_box)
# frame0 = cv2.cvtColor(np.array(sct_img), cv2.COLOR_BGR2GRAY)
frame1 = sct.grab(bounding_box)
# frame1 = cv2.cvtColor(np.array(sct_img), cv2.COLOR_BGR2GRAY)

while True:
    frame0,frame1=np.array(frame0),np.array(frame1)
